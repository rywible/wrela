...
3. Walk and look through native input. Exercise movement and contextual interaction; use Soundstage for object, lighting and motion controls. Refresh the accessibility tree after actions.
...
6. Run `workshop-study` to find flaws hidden by a single angle or lighting setup. Inspect the returned real PNGs; do not substitute generated concept images for renderer evidence.
7. Run `stagectl verifyFields` after field/compiler changes, and `./scripts/validate` after input, collision, capture, or command changes.
...
./scripts/stagectl subject seed
./scripts/stagectl pause true
./scripts/stagectl camera --orbit 0.5 --elevation 0.2 --distance 6.8
./scripts/stagectl lighting morning
./scripts/stagectl capture --label before
# Edit the shader source.
./scripts/stagectl reloadShaders
./scripts/stagectl capture --label after
```
...
./scripts/gardenctl capture --label field-query
```
...
`replay` consumes an ordered JSON array of actions. The checked-in `arrival-replay.json` pauses, captures, moves with collision, steps time, and restores a live arrival view. It is deterministic at the simulation-command level; it does not synthesize desktop input or promise floating-point lockstep across devices.
...
Game actions include `status`, `lighting`, `camera`, `move`, `pause`, `step`, `parameters`, `query`, `capture`, `clearMetrics`, `reloadShaders` and game-specific interactions/saves. Object/view/sky/field verification belong to `stagectl`; use its `catalog`. Cave uses `gamectl --project cave` and `.cave`; Soundstage uses `.soundstage`.

Supported parameter ranges: exposure 0.5–1.5; wetness 0–1; seed pod scale 0.25–4. Invalid values fail before applying any part of that parameter command. Shader pipelines are replaced only after all candidate pipelines compile.
