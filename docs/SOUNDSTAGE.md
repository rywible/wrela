...
The native panel has subject and rig selectors, tree shape controls, material
controls, views, scale reference, turntable, capture and study saving/loading.
Scroll the panel for more controls. Drag the viewport to orbit, scroll to zoom,
R to fit, P to play/pause, F to capture, H to hide/show controls, Tab to switch sky
...
overrides. `assetReload` reloads that canonical saved path. For a newly imported
file, save it first or reload its original path with `assetLoad`.
...
failed capture. Reports include an HTML wipe viewer with zoom, condition selection,
