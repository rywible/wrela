...
public enum AnatomyPrimitive: String, Codable, Sendable { case ellipsoid, capsule, loft }
...
public struct AnatomyElement: Codable, Equatable, Sendable {
  ...
  public var sections:[LoftSection]?
  public var color:V3
  ...
  public init(id:String,region:String,primitive:AnatomyPrimitive = .ellipsoid,
    ...
    color:V3 = V3(repeating:0.6),jointWeights:[String:Float] = [:],cutBlend:Float?=nil,sections:[LoftSection]?=nil) {
    ...
    self.cutBlend=cutBlend;self.sections=sections
  }
  ...
  public var shape:Shape {
    ...
    case .loft:return Shape.loft(SectionLoft(sections ?? [])).rotated(orientation).moved(center)
    }
  }
  public func validate(joints:Set<String>? = nil) throws {
    ...
    if primitive == .loft {
      try CraftError.require(radius == V3(repeating:1) && end == .zero,path,"Lofts use metre sections, unit radius and zero end; orient with rotation")
      try SectionLoft(sections ?? []).validate()
    } else {try CraftError.require(sections == nil,path,"Sections apply only to lofts")}
  ...
  public func coordinate(at point:V3)->V3 {
    ...
    if primitive == .loft {return SectionLoft(sections ?? []).coordinate(at:orientation.inverse.act(point-center))}
  ...
  public func point(at coordinate:V3)->V3 {
    ...
    if primitive == .loft {return center+orientation.act(SectionLoft(sections ?? []).point(at:coordinate))}
...
public struct AnatomySource: Codable, Equatable, Sendable {
  ...
  public var sampling:SurfaceSampling?
  public init(id:String,part:String,elements:[AnatomyElement],resolution:Int=64,
    material:Int=7,roughness:Float=0.65,metallic:Float=0,replaces:[String]=[],skinBlend:Float=0,sampling:SurfaceSampling?=nil) {
    ...
    self.skinBlend=skinBlend;self.sampling=sampling
  }
  private enum CodingKeys:String,CodingKey {case id,part,elements,resolution,material,roughness,metallic,replaces,skinBlend,sampling}
  public init(from decoder:Decoder) throws {
    ...
    sampling=try c.decodeIfPresent(SurfaceSampling.self,forKey:.sampling)
  }
  ...
  /// Prepare immutable primitive coefficients once for a batch of field/skin
  /// queries. Recreate this sampler after source edits.
  public func preparedSampler()->AnatomySampler {AnatomySampler(self)}
  public func sample(at point:V3)->AnatomySample {preparedSampler().sample(at:point)}
  ...
  public func changes(from previous:AnatomySource)->AnatomyChangeReport {
    ...
    for old in previous.elements {
      ...
      if new.sections?.map(\.id) != old.sections?.map(\.id) {reasons.append("Element \(old.id) changed section identities or order")}
