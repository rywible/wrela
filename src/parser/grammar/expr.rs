...
fn lhs(p: &mut Parser) -> Option<crate::parser::CompletedMarker> {
    let m = p.start();
    match p.peek() {
        SyntaxKind::Minus | SyntaxKind::NotKw | SyntaxKind::AwaitKw | SyntaxKind::BitwiseNot => {
            p.bump();
...
fn infix_binding_power(kind: SyntaxKind) -> Option<(u8, u8)> {
    match kind {
        ...
        | SyntaxKind::LessEq
        | SyntaxKind::Greater
        | SyntaxKind::GreaterEq => Some((13, 14)),
...
fn parse_call(p: &mut Parser, lhs: crate::parser::CompletedMarker) -> crate::parser::CompletedMarker {
    let m = lhs.precede(p);
