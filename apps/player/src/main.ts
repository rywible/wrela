...
import { WinterPlayer } from "./winter-game";
import { winterStyle } from "./winter-style";
...
async function setGraphics(profile: RenderQuality) {
  ...
  if (!renderer || preparing || winter?.busy) throw new Error("Wait for the player to finish preparing");
  ...
  await transition(
    "Changing graphics",
    async () => {
      const scene = winter?.active ? winter.scene() : host.extract(camera);
      renderer.dispose();
      try {
...
async function travel(x: number, z: number) {
  if (winter?.active) throw new Error("Return to the viewer before using viewer travel");
  return transition(
...
async function openSubject(id: string) {
  try {
    await transition(
      "Preparing",
      () => host.prepare(id, "neutral-stage", "review"),
      () => {
...
async function start() {
  try {
    ...
    project = parseProject(embedded ? JSON.parse(embedded) : referenceProject());
    ...
    host = new BrowserSceneHost(project, {
      compile,
      ...(compiler ? { generateTerrain: compiler.generateTerrain } : {}),
    });
    ...
    explore.hidden = !winter.supported();
    ...
    (window as any).wrelaPlayer = {
      ...
      inspect: () => ({
        ...
        playing: winter?.active
          ? !winter.rules.paused && ["exploring", "returning"].includes(winter.rules.inspect().phase)
          : playing,
        camera: structuredClone(winter?.active ? winter.camera : camera),
        preparing: preparing || !!winter?.busy,
...
canvas.addEventListener("pointerdown", (e) => {
  if (preparing || disposed || winter?.active) return;
...
canvas.addEventListener("pointermove", (e) => {
  if (!drag || preparing || disposed || winter?.active) return;
