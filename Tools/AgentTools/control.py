...
ROOT = Path(os.environ['WRELA_CONTROL_ROOT']) if os.environ.get('WRELA_CONTROL_ROOT') else Path(__file__).resolve().parents[2] / os.environ.get('SANCTUARY_CONTROL_ROOT', '.sanctuary')

def command(action, **kwargs):
    status_path = ROOT / 'status.json'
    if not status_path.exists():
        raise RuntimeError('Start the selected game or Soundstage first')
    status = json.loads(status_path.read_text())
    if status.get('stopped') or time.time() - status.get('heartbeat', 0) > 10:
        raise RuntimeError('The selected app is not responding; launch or restore its window first')
...
def main():
    ...
    shape=sub.add_parser('shape');shape.add_argument('--set',dest='edits',nargs=2,action='append',default=[])
    for key in ['height','width','trunk','spread','leaves','headSize','cheeks','bodyRoundness','bodyLength','earLength','earWidth','earSplay','eyeSize','eyeSpacing','pawSize','tailSize']:shape.add_argument('--'+key,type=float)
    ...
    for key in ['x','z']:stimulus.add_argument('--'+key,type=float)
    for key in ['running','visible']:stimulus.add_argument('--'+key,choices=['true','false'])
