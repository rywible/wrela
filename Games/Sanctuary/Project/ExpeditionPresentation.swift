...
final class ExpeditionPresentation {
  ...
  init(graphics: MetalRenderer, terrain: Terrain) throws {
    ...
    source = try AssetSource.load("frostling")
    creature = try source.compile().map(graphics.upload)
  ...
  func items(_ state: Expedition) -> [RenderItem] {
    ...
    if state.phase != .carrying {
      let actor = state.creatureState
      let motion = SanctuaryProject.motion(source.motion ?? MotionParameters())
      let poses = PartRig.matrices(
        source.joints,
        poses: motion.poses(
          time: actor.time, phase: actor.phase, alert: actor.fear, gaze: actor.gaze))
      let height = actor.phase.map { CreatureMotion.flight($0) * motion.height } ?? 0
      let p = actor.position
      for batch in creature {
        let tint = batch.sourceInstances[0].tint
        var instance = Instance(
          position: V3(p.x, terrain.height(p.x, p.y) + height, p.y), yaw: actor.yaw,
          tint: V3(tint.x, tint.y, tint.z), kind: tint.w)
        instance.model *= poses[batch.name] ?? matrix_identity_float4x4
