...
impl<'a> Checker<'a> {
    ...
    fn check_check_result_inline(
        ...
        if !matches!(&body.exprs[value], Expr::GivenCall { .. }) {
            return;
        }
...
fn collect_declared_names(body: &Body, stmt_id: Idx<Stmt>, out: &mut HashSet<SmolStr>) {
    match &body.stmts[stmt_id] {
        Stmt::Let { name, .. } | Stmt::Capture { name, .. } | Stmt::For { name, .. } => {
...
mod tests {
    ...
    fn enforces_function_snake_and_verb() {
        let errors = naming_errors("to helperThing() -> Integer:\n    return 1\n");
        assert!(
            errors
                .iter()
    ...
    #[test]
    fn enforces_top_level_check_subject_statement_shape() {
        let errors =
            naming_errors("check is_ready(value: Integer) -> Boolean:\n    return value > 0\n");
        assert!(errors.iter().any(|err| {
            matches!(
                err,
                NamingError::TopLevelCheckName { name, .. } if name == "is_ready"
            )
        }));
    }
    ...
    fn enforces_result_prefix_and_error_type_suffix() {
        let errors = naming_errors(
            "to fetch_user() -> Result[User, network_failure]:\n    return error network_failure\n",
        );
        assert!(errors.iter().any(|err| {
            matches!(
                err,
    ...
    fn enforces_boolean_prefix_for_param_and_local() {
        let errors = naming_errors(
            "to run_check(ready: Boolean) -> Integer:\n    flag = true\n    if flag:\n        return 1\n    return 0\n",
        );
        assert!(errors.iter().any(|err| {
            matches!(
                err,
    ...
    fn flags_immediate_single_use_check_storage() {
        let errors = naming_errors(
            "check account_is_ready(v: Integer) -> Boolean:\n    return v > 0\n\n\
             to run() -> Integer:\n    is_ready = account_is_ready given(1)\n    if is_ready:\n        return 1\n    return 0\n",
        );
        assert!(errors.iter().any(|err| {
