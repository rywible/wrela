...
fn rewrite_private_call_targets(body: &mut Body, renamed: &HashMap<SmolStr, SmolStr>) {
    ...
    for raw in 0..body.exprs.len() {
        ...
        match &body.exprs[idx] {
            Expr::Call { callee, .. } | Expr::TypeApply { callee, .. } => {
...
impl ProjectLoader {
    ...
    fn enforce_entrypoint(&mut self, entry_name: &SmolStr) {
        ...
        if !has_run {
            self.errors.push(ProjectError {
                ...
                message: "entry module must define 'fn run() -> Type'".to_string(),
                span: SourceSpan::from((0usize, 0usize)),
            });
            return;
        }
        ...
        for module_name in module_names {
            ...
            for use_site in &module.uses {
                ...
                if is_host_http_module(use_site.module.as_str())
                    && !is_network_integration_module(module.name.as_str())
                {
                    self.errors.push(ProjectError {
                        kind: ProjectDiagKind::LoadError,
                        path: module.path.clone(),
    ...
    fn enforce_intrinsic_boundary(&mut self) {
        let mut module_names: Vec<SmolStr> = self.modules.keys().cloned().collect();
        module_names.sort_by(|a, b| a.as_str().cmp(b.as_str()));
        ...
        for module_name in module_names {
            ...
            if intrinsic_boundary_allowed_path(&module.path) {
                continue;
            }
            for (_, func) in module.module.functions.iter() {
                let Some(body) = &func.body else {
            ...
            if let Some(root_body) = &module.root_body {
                for raw in 0..root_body.exprs.len() {
                    let expr_id = Idx::new(raw);
                    if let Expr::Variable(name) = &root_body.exprs[expr_id]
                        && name.as_str().starts_with("__wr_")
                    {
                        self.errors.push(ProjectError {
                            kind: ProjectDiagKind::LoadError,
                            ...
                            span: span_from_range(root_body.expr_span(expr_id)),
                        });
                    }
                }
            }
    ...
    fn enforce_network_boundary_policy(&mut self, effects: &[ClassifiedFunctionEffect]) {
        ...
        for effect in effects {
            ...
            if is_network_integration_module(effect.entry.module.as_str()) {
                continue;
            }
            let Some(module) = self.modules.get(&effect.entry.module) else {
                continue;
            ...
            self.errors.push(ProjectError {
                ...
                source: module.source.clone(),
                message: format!(
                    "network function '{}::{}' uses external network I/O outside infrastructure/integrations or frontend/integrations. help: move this call into src/infrastructure/integrations/** or src/frontend/integrations/** and invoke it through a domain/application interface",
...
fn effect_for_builtin_symbol(name: &str) -> FunctionEffect {
    match name {
        "__wr_env_get" | "__wr_clock_ns" | "__wr_fs_read_bytes" | "__wr_runtime_cpu_count" => {
            FunctionEffect::HostRead
        }
        "__wr_pool_size" | "__wr_pool_rr" | "__wr_pool_queue_len" | "__wr_actor_mailbox_len" => {
            FunctionEffect::HostRead
        }
        "__wr_env_set" | "__wr_fs_write_bytes" | "__wr_sleep_ms" | "__wr_runtime_configure" => {
            FunctionEffect::HostWrite
        }
        _ if name.starts_with("__wr_actor_")
            || name.starts_with("__wr_pending_")
            || (name.starts_with("__wr_pool_")
                && !matches!(
                    name,
                    "__wr_pool_size" | "__wr_pool_rr" | "__wr_pool_queue_len"
                )) =>
        {
            FunctionEffect::HostWrite
        }
...
fn effect_for_imported_symbol(module_name: &str, function_name: &str) -> FunctionEffect {
    ...
    if module_name == "host/time" {
        if function_name.contains("sleep") {
            return FunctionEffect::HostWrite;
        }
        return FunctionEffect::HostRead;
    }
    ...
    if module_name == "runtime/actor" {
        if function_name == "get_mailbox_length" {
            return FunctionEffect::HostRead;
        }
        if function_name == "pause"
            || function_name == "resume"
            || function_name == "pause_wait"
            || function_name == "pause_and_wait"
            || function_name.starts_with("fire_burst")
        {
            return FunctionEffect::HostWrite;
        }
        return FunctionEffect::Pure;
    }
    if module_name == "runtime/pool" {
        if function_name == "auto_size"
            || function_name == "size"
            || function_name == "round_robin"
            || function_name == "queue_len"
        {
            return FunctionEffect::HostRead;
        }
        return FunctionEffect::Pure;
...
fn is_frontend_integration_module(module_name: &str) -> bool {
...
fn intrinsic_boundary_allowed_path(path: &Path) -> bool {
    let normalized = path.to_string_lossy().replace('\\', "/");
    let in_tree = |segment: &str| {
    ...
    in_tree("language/stdlib")
        || in_tree("language/spec")
        || in_tree("compiler/tests")
        || in_tree("tests")
        || in_tree("target/wrela_tests")
        || in_tree("target/wrela_mutation")
}
            ...
            Stmt::Match {
                ...
                for case in cases {
                    if let Some(guard) = case.guard {
                        collect_called_functions_in_expr(body, guard, called);
                    }
...
fn collect_called_functions_in_expr(
    ...
    match &body.exprs[expr_id] {
        ...
        Expr::Index { object, index, .. } => {
            collect_called_functions_in_expr(body, *object, called);
            collect_called_functions_in_expr(body, *index, called);
        }
        Expr::List(items) => {
            for item in items {
        ...
        Expr::Closure {
            ...
            collect_called_functions_in_expr(body, *closure_body, called);
        }
    }
}
    ...
    for stmt_id in stmts {
        let found = match &body.stmts[*stmt_id] {
            ...
            Stmt::Match {
                ...
                for case in cases {
                    found = found.or_else(|| {
                        case.guard.and_then(|guard| {
                            find_disallowed_domain_async_keyword_in_expr(body, guard)
                        })
                    });
                    found = found.or_else(|| {
...
fn find_disallowed_domain_async_keyword_in_expr(
    ...
    match &body.exprs[expr_id] {
        ...
        Expr::Index { object, index, .. } => {
            find_disallowed_domain_async_keyword_in_expr(body, *object)
                .or_else(|| find_disallowed_domain_async_keyword_in_expr(body, *index))
        }
        Expr::List(items) => {
            for item in items {
        ...
        Expr::Closure {
            body: closure_body, ..
        } => find_disallowed_domain_async_keyword_in_expr(body, *closure_body),
    }
}
...
fn classify_module_layer(module_name: &str) -> ModuleLayer {
    ...
    match (parts.next(), parts.next()) {
        ...
        (Some("presentation"), _) => ModuleLayer::Presentation,
        (Some("application"), Some("composition")) => ModuleLayer::CompositionRoot,
        (Some("application"), _) => ModuleLayer::Application,
        (Some("frontend"), Some("integrations")) => ModuleLayer::FrontendIntegrations,
...
fn architecture_import_allowed(source: ModuleLayer, target: ModuleLayer) -> bool {
    use ModuleLayer::{
        Application, CompositionRoot, Domain, FrontendIntegrations, Infrastructure, Other,
        Presentation,
    };
    match (source, target) {
        (Other, _) | (_, Other) => true,
        (CompositionRoot, _) => true,
        (_, FrontendIntegrations) => matches!(source, Application | CompositionRoot),
        (Domain, Domain) => true,
        (Domain, _) => false,
        (Presentation, Domain | Presentation) => true,
        (Presentation, _) => false,
        (Application, Domain | Application) => true,
        (Application, _) => false,
        (FrontendIntegrations, Domain | Presentation) => true,
        (FrontendIntegrations, _) => false,
...
fn host_import_allowed(source: ModuleLayer) -> bool {
    !matches!(
        source,
        ModuleLayer::Domain | ModuleLayer::Presentation | ModuleLayer::Application
    )
}
...
fn architecture_layer_violation_message(
    ...
    let recipe = if target == ModuleLayer::FrontendIntegrations {
        "frontend/integrations/* can only be imported by application/* or application/composition/*. move this import to an application service or the composition root"
    } else {
        match source {
            ModuleLayer::Domain => {
                "domain/* can only depend on domain/*. move orchestration or adapters into presentation/application/infrastructure"
            }
            ModuleLayer::Presentation => {
                "presentation/* can only depend on domain/* or presentation/*. move this dependency behind a domain-facing contract"
            }
            ModuleLayer::Application => {
                "application/* can depend on domain/*, application/*, and frontend/integrations/*. move infrastructure wiring into application/composition"
            }
            ModuleLayer::FrontendIntegrations => {
                "frontend/integrations/* should depend on domain/* or presentation/* contracts only. move orchestration into application/* or application/composition/*"
            }
            ModuleLayer::Infrastructure => {
                "infrastructure/* should depend on domain/* abstractions and infrastructure/* internals. keep orchestration in application/composition"
            }
            ModuleLayer::CompositionRoot => {
                "application/composition/* can import all layers; this error indicates a classification mismatch"
            }
            ModuleLayer::Other => {
                "place this module under src/domain, src/presentation, src/application, src/frontend/integrations, or src/infrastructure for explicit layering"
            }
        }
    };
    format!(
...
fn host_import_violation_message(source: ModuleLayer, imported_module: &str) -> String {
    format!(
        "{} modules cannot import host module '{}'. help: host imports are only allowed in infrastructure/*, frontend/integrations/*, and application/composition/*. route host access through domain/presentation contracts and wire adapters in application/composition",
        layer_label(source),
        imported_module
    )
}
...
fn layer_label(layer: ModuleLayer) -> &'static str {
    match layer {
        ...
        ModuleLayer::Presentation => "presentation",
        ModuleLayer::Application => "application",
        ModuleLayer::CompositionRoot => "application/composition",
        ModuleLayer::FrontendIntegrations => "frontend/integrations",
...
fn collect_used_external_names(module: &Module) -> HashMap<SmolStr, TextRange> {
    ...
    for (_, class) in module.classes.iter() {
        let class_type_params: HashSet<SmolStr> =
    ...
    for (_, en) in module.enums.iter() {
        let enum_type_params: HashSet<SmolStr> =
    ...
    for (_, interface) in module.interfaces.iter() {
        let interface_type_params: HashSet<SmolStr> = interface
            .type_params
            .iter()
    ...
    for stmt_id in stmts {
        ...
        match stmt {
            ...
            Stmt::For {
                ...
                scope.enter();
                scope.insert(value_name.clone());
                if let Some(key_name) = key_name {
                    scope.insert(key_name.clone());
                }
                if let Some(index_name) = index_name {
                    scope.insert(index_name.clone());
                }
            ...
            Stmt::Match {
                ...
                for case in cases {
                    ...
                    if let Some(guard) = case.guard {
                        collect_used_in_expr(body, guard, scope, used);
                    }
...
fn collect_used_in_expr(
    ...
    match expr {
        Expr::Literal(_) => {}
        Expr::Variable(name) => {
            if name != "self" && name != "Self" && !scope.contains(name) {
        ...
        Expr::Index { object, index, .. } => {
            collect_used_in_expr(body, *object, scope, used);
            collect_used_in_expr(body, *index, scope, used);
        }
        Expr::List(items) => {
            for item in items {
        ...
        Expr::Closure {
            ...
            collect_used_in_expr(body, *closure_body, scope, used);
        }
    }
}
...
mod tests {
    ...
    fn test_load_project_with_use() {
        ...
        write_temp(
            &entry_path,
            "use foo from bar\n\nfn run() -> Integer { return f() }\n\nfn f() -> Integer { return foo() }\n",
        );
        write_temp(&mod_path, "fn foo() -> Integer { return 1 }\n");
    ...
    fn test_missing_import_is_error() {
        ...
        let entry_path = base.join("src").join("main.wr");
        write_temp(&entry_path, "fn run() -> Integer { return foo() }\n");
    ...
    fn test_entry_requires_run() {
        ...
        let entry_path = base.join("src").join("main.wr");
        write_temp(&entry_path, "fn f() -> Integer { return 1 }\n");
    ...
    fn test_run_only_in_entry_module() {
        ...
        write_temp(&mod_path, "fn run() -> Integer { return 2 }\n");
    ...
    fn test_module_layer_classification_is_deterministic() {
        ...
        assert_eq!(
            classify_module_layer("presentation/shell"),
            ModuleLayer::Presentation
        );
        assert_eq!(classify_module_layer("domain/user"), ModuleLayer::Domain);
        assert_eq!(
            classify_module_layer("frontend/integrations/browser"),
            ModuleLayer::FrontendIntegrations
        );
        assert_eq!(
    ...
    #[test]
    fn test_architecture_import_policy_by_layer() {
        assert!(architecture_import_allowed(
            ModuleLayer::Domain,
            ModuleLayer::Domain
        ));
        assert!(!architecture_import_allowed(
    ...
    fn test_host_import_policy_by_layer() {
        ...
        assert!(!host_import_allowed(ModuleLayer::Presentation));
        assert!(!host_import_allowed(ModuleLayer::Application));
        assert!(host_import_allowed(ModuleLayer::FrontendIntegrations));
    ...
    #[test]
    fn test_layer_violation_messages_include_remediation() {
        let architecture_message = architecture_layer_violation_message(
            ModuleLayer::Presentation,
            ModuleLayer::FrontendIntegrations,
            "frontend/integrations/router",
        );
        assert!(architecture_message.contains("help:"));
        assert!(architecture_message.contains(
            "frontend/integrations/* can only be imported by application/* or application/composition/*"
        ));

        let host_message =
            host_import_violation_message(ModuleLayer::Presentation, "host/http/client");
        assert!(host_message.contains("help:"));
        assert!(host_message.contains("infrastructure/*, frontend/integrations/*"));
    }

    #[test]
    fn test_network_integration_module_detection() {
    ...
    fn test_function_effect_classification_is_deterministic() {
        ...
        write_temp(
            &entry_path,
            "use try_to_http_call from host/http\n\nfn pure_math() -> Integer { return 1 + 2 }\n\nfn call_api() -> Integer {\n    headers = {}\n    response = try_to_http_call(\"svc\", \"ep\", \"GET\", \"http://127.0.0.1:9/ping\", headers, \"\", 200) ?? \"fallback\"\n    if response == \"fallback\" { return 0 }\n    return 200\n}\n\nfn run() -> Integer { return pure_math() + call_api() }\n",
        );
