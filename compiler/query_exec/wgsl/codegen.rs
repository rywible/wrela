...
fn emit_query_helpers(
    ...
    if matches!(
        ...
        writeln!(
            out,
            "  if (dispatch_config.accel_node_count == 0u) {{ return world_trace_ray_dense(ray); }}"
        )
        .ok();
        ...
        writeln!(
            out,
            "  if (!wr_push_accel_node(ray, dispatch_config.accel_root_index, 0.0, &stack_len, &stack_nodes, &stack_starts)) {{ return world_trace_ray_dense(ray); }}"
        )
        .ok();
        writeln!(out, "  loop {{").ok();
        ...
        writeln!(
            out,
            "      if (!wr_push_accel_node(ray, child_index, 0.0, &stack_len, &stack_nodes, &stack_starts)) {{ return world_trace_ray_dense(ray); }}"
        )
        .ok();
        writeln!(out, "    }}").ok();
        writeln!(out, "  }}").ok();
...
fn emit_world_trace_helpers(
    ...
    writeln!(
        out,
        "  if (node_index >= dispatch_config.accel_node_count || *stack_len >= 128u) {{ return false; }}"
    )
    .ok();
    ...
    writeln!(out, "}}\n").ok();
    writeln!(out, "fn world_trace_ray_dense(ray: RayQuery) -> Hit3 {{").ok();
    writeln!(
        out,
    ...
    writeln!(
        out,
        "    let hit = trace_shape_for_index(world_shapes.values[index], ray.origin, ray.direction, 0.0, ray.max_distance, ray.min_step, ray.hit_epsilon, ray.max_steps);"
    )
    .ok();
    writeln!(
        out,
...
fn emit_analytic_intersection_helpers(out: &mut String) -> Result<(), QueryExecError> {
    writeln!(
        out,
...
fn scene_named_arg_constant_opt(
    ops: &DirectQueryOps<'_>,
...
fn kernel_value_f32(value: &KernelValue, label: &str) -> Result<f32, QueryExecError> {
    match value {
...
fn kernel_value_vec3(value: &KernelValue, label: &str) -> Result<[f32; 3], QueryExecError> {
    match value {
...
fn analytic_transform_point_expr(
    kind: TransformKind,
    value: &KernelValue,
    rendered_value: &str,
    point_expr: &str,
...
fn analytic_transform_vector_expr(
    kind: TransformKind,
    value: &KernelValue,
    rendered_value: &str,
    vector_expr: &str,
