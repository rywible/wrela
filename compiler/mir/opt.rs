            ...
            match stmt {
                ...
                Stmt::ActorFire { target, args, .. } => {
                    if let CallTarget::Method { receiver, .. } = target {
                        bump_use(receiver, &mut use_counts);
                    }
                    for arg in args {
                        bump_use(arg, &mut use_counts);
                    }
                }
...
fn bump_local_reads_in_stmt(stmt: &Stmt, local_use_counts: &mut [u32]) {
    match stmt {
        ...
        Stmt::ActorFire { target, args, .. } => {
            ...
            for arg in args {
                bump_local_reads_in_value(arg, local_use_counts);
            }
        }
...
pub fn inline_small_pure_functions(module: &mut MirModule, graph: &CallGraph) -> usize {
    ...
    fn remap_value(
        ...
        match value {
            ...
            Value::Local(id) => Value::Local(
                *local_map
                    .get(id)
                    .unwrap_or_else(|| panic!("ICE: inliner local map missing {id:?}")),
            ),
            Value::Temp(id) => Value::Temp(
                *temp_map
                    .get(id)
                    .unwrap_or_else(|| panic!("ICE: inliner temp map missing {id:?}")),
            ),
        }
    }
    ...
    fn remap_place(
        ...
        match place {
            Place::Local(id) => Place::Local(
                *local_map
                    .get(id)
                    .unwrap_or_else(|| panic!("ICE: inliner local map missing {id:?}")),
            ),
            Place::Temp(id) => Place::Temp(
                *temp_map
                    .get(id)
                    .unwrap_or_else(|| panic!("ICE: inliner temp map missing {id:?}")),
            ),
        ...
        for block in &mut func.blocks {
            ...
            for stmt in old_stmts {
                ...
                for (param, arg) in candidate.func.params.iter().zip(args.iter()) {
                    let mapped_param = *local_map.get(param).unwrap_or_else(|| {
                        panic!("ICE: inliner param local map missing {param:?}")
                    });
            ...
            for stmt in &mut block.stmts {
                ...
                } = target
                {
                    if method_id.is_none()
                        && let Some(interface_target) =
                            resolve_unique_interface_dispatch_target(method, &dispatch)
                    {
                        let receiver_value = receiver.clone();
                        let mut rewritten_args = Vec::with_capacity(args.len() + 1);
                        rewritten_args.push(receiver_value);
                        rewritten_args.extend(args.iter().cloned());
                        *args = rewritten_args;
                        *target = CallTarget::Function(interface_target);
                    }
                }
...
fn resolve_unique_interface_dispatch_target(
    method: &SmolStr,
    dispatch: &HashMap<SmolStr, InterfaceDispatchInfo>,
) -> Option<SmolStr> {
    if method.as_str().contains('.') {
        return None;
    }

    let mut matched: Option<SmolStr> = None;
    for target_name in dispatch.keys() {
        let Some((_interface_name, target_method)) = target_name.as_str().rsplit_once('.') else {
            continue;
        };
        if target_method != method.as_str() {
            continue;
        }
        if matched.is_some() {
            return None;
        }
        matched = Some(target_name.clone());
    }
    matched
}
    ...
    for block in &func.blocks {
        for stmt in &block.stmts {
            match stmt {
                ...
                Stmt::ActorFire { target, args, .. } => {
                    if let CallTarget::Method { receiver, .. } = target {
                        collect_disallowed_value_use(receiver, &mut replaceable);
                    }
                    for arg in args {
                        collect_disallowed_value_use(arg, &mut replaceable);
                    }
                }
                Stmt::IterNext { iter, .. } => {
    ...
    for block in &func.blocks {
        for stmt in &block.stmts {
            match stmt {
                ...
                Stmt::ActorFire { target, args, .. } => {
                    if let CallTarget::Method { receiver, .. } = target {
                        collect_value(receiver, &mut used);
                    }
                    for arg in args {
                        collect_value(arg, &mut used);
                    }
                }
...
fn insert_rc(func: &mut MirFunction) {
    ...
    let succs = block_successors(func);
    let preds = block_predecessors(func, &succs);
    let phi_edge_uses = collect_phi_edge_uses(func, locals_len);
    let phi_defs = collect_phi_defs(func, locals_len, total);
    ...
    for (block_idx, block) in func.blocks.iter_mut().enumerate() {
        ...
        // Phi sources conceptually move into their phi destinations on block entry.
        // Keep them out of this block's local finalization set; predecessor edge liveness
        // handles releasing non-selected predecessors.
        for stmt in block.stmts.iter() {
            let Stmt::Phi { sources, .. } = stmt else {
                break;
            };
            for (_, value) in sources {
...
fn collect_phi_edge_uses(func: &MirFunction, locals_len: usize) -> Vec<Vec<Vec<usize>>> {
    let mut edge_uses = vec![vec![Vec::new(); func.blocks.len()]; func.blocks.len()];
    for (succ_idx, block) in func.blocks.iter().enumerate() {
        for stmt in &block.stmts {
            ...
            for (pred, value) in sources {
                if let Some(idx) = value_idx(value, locals_len) {
                    edge_uses[pred.0][succ_idx].push(idx);
                }
            }
        }
    }
    for pred_row in &mut edge_uses {
        for uses in pred_row {
            uses.sort_unstable();
            uses.dedup();
        }
    }
...
fn collect_phi_defs(func: &MirFunction, locals_len: usize, total: usize) -> Vec<Vec<bool>> {
    let mut defs = vec![vec![false; total]; func.blocks.len()];
    for (block_idx, block) in func.blocks.iter().enumerate() {
        for stmt in &block.stmts {
            let Stmt::Phi { place, .. } = stmt else {
                break;
            };
            defs[block_idx][place_idx(place, locals_len)] = true;
        }
    }
    defs
}
...
fn liveness(
    ...
    loop {
        ...
        for b in 0..blocks_len {
            ...
            for succ in &succs[b] {
                for i in 0..total {
                    out[i] |= live_in[*succ][i] && !phi_defs[*succ][i];
                }
                for idx in &phi_edge_uses[b][*succ] {
                    out[*idx] = true;
                }
            }
...
mod tests {
    ...
    fn test_inline_small_pure_function() {
        let input = "fn add(a: Integer, b: Integer) -> Integer {
    return a + b
}
...
fn run() -> Integer {
    ...
    #[test]
    fn rc_phi_source_is_not_dropped_in_merge_block() {
        let span = TextRange::new(0.into(), 0.into());
        let mut func = MirFunction {
            ...
            locals: vec![
                ...
                Local {
                    name: "bind_address#ssa1".into(),
                    mutable: false,
                    ty: MirType::String,
                },
                Local {
                    name: "bind_address#ssa2".into(),
                    mutable: false,
                    ty: MirType::String,
                },
                Local {
                ...
                Local {
                    name: "branch_cond".into(),
                    mutable: false,
                    ty: MirType::Boolean,
                },
                Local {
                    name: "noop_result".into(),
                    mutable: false,
                    ty: MirType::Integer,
                },
            ],
            ...
            blocks: vec![
                BasicBlock {
                    stmts: vec![
                        Stmt::Assign {
                            place: Place::Local(LocalId(0)),
                            value: Rvalue::Use(Value::Const(Literal::String(
                                "0.0.0.0:8080".into(),
                            ))),
                            span,
                        },
                ...
                BasicBlock {
                    stmts: vec![Stmt::Assign {
                        ...
                        value: Rvalue::Use(Value::Const(Literal::String("[::]:8080".into()))),
                        span,
                    }],
                ...
                BasicBlock {
                    stmts: vec![
                        Stmt::Phi {
                            ...
                            sources: vec![
                                (BlockId(1), Value::Local(LocalId(1))),
                                (BlockId(2), Value::Local(LocalId(0))),
                            ],
                            original: LocalId(2),
                            span,
                        },
                        Stmt::Assign {
                            place: Place::Local(LocalId(5)),
                            value: Rvalue::Call {
                                kind: CallKind::Sync,
                                target: CallTarget::Function("noop".into()),
                                args: vec![],
                            },
                            span,
                        },
                        ...
                        Stmt::Assign {
                            ...
                            value: Rvalue::Call {
                                kind: CallKind::Sync,
                                target: CallTarget::Function("__wr_map_set".into()),
                                args: vec![
                                    Value::Local(LocalId(3)),
                                    Value::Const(Literal::String("bind_address".into())),
        ...
        let drops_bind_address_source = merge_block.stmts.iter().any(|stmt| {
            matches!(
                stmt,
                Stmt::RcDec {
                    value: Value::Local(LocalId(0)),
        ...
        assert!(
            !drops_bind_address_source,
            "phi source local must not be decremented in the merge block before use"
        );
    }
