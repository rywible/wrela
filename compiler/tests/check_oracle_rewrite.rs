...
"#;
    ...
    let check = checkir
        .checks
        .iter()
        .find(|check| check.name.as_str() == "risky")
...
fn checkir_shape_hash_is_canonical_for_commutative_forms() {
    let source = r#"
check left(a: Integer, b: Integer) -> Boolean:
    return (a + b) > 0

check right(a: Integer, b: Integer) -> Boolean:
    return (b + a) > 0
...
"#;
    ...
    let left = checkir
        .checks
        .iter()
        .find(|check| check.name.as_str() == "left")
        .expect("left");
    let right = checkir
        .checks
        .iter()
        .find(|check| check.name.as_str() == "right")
        .expect("right");
