...
"#;
    ...
    assert_eq!(valid_tokens[7], Token::Otherwise);
...
"#;
    ...
    assert!(errors.is_empty(), "{:?}", errors);
    let tokens = strip_spans(tokens);
    assert!(tokens.contains(&Token::Otherwise));
    assert!(
        tokens.iter().filter(|t| **t == Token::Dedent).count() >= 2,
        "expected dedents for match and function blocks"
    );
}
...
"#;
    ...
    assert_eq!(valid_tokens[9], Token::Fire);
    assert_eq!(
        valid_tokens[10],
        Token::Identifier(SmolStr::new("some_task"))
    );
    assert_eq!(valid_tokens[11], Token::LParen);
    assert_eq!(valid_tokens[12], Token::RParen);

    // "so: inline" should be gone.
    // x = 1
    assert_eq!(valid_tokens[13], Token::Identifier(SmolStr::new("x")));
    assert_eq!(valid_tokens[14], Token::Equals);
    assert_eq!(valid_tokens[15], Token::Integer(1, SmolStr::new("1"))); // Changed from Number(1.0)

    // "so: block" should be gone.
    // return x
    assert_eq!(valid_tokens[16], Token::Return);
    assert_eq!(valid_tokens[17], Token::Identifier(SmolStr::new("x")));
}
