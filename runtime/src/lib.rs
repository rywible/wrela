...
static ENTITY_REGISTRY: OnceLock<Mutex<entity::EntityRegistry>> = OnceLock::new();

fn entity_registry() -> &'static Mutex<entity::EntityRegistry> {
    ENTITY_REGISTRY.get_or_init(|| Mutex::new(entity::EntityRegistry::new()))
}

static PHYSICS_WORLD: OnceLock<Mutex<physics::world::PhysicsWorld>> = OnceLock::new();

fn physics_world() -> &'static Mutex<physics::world::PhysicsWorld> {
    PHYSICS_WORLD.get_or_init(|| Mutex::new(physics::world::PhysicsWorld::new()))
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_create_listener(configuration: Value) -> Value {
    runtime_startup_trace("wr_game_session_create_listener: enter");
    let result = web::game_session_create_listener(configuration);
    if let Some(handle) = int_value(result) {
        runtime_startup_trace(format!(
            "wr_game_session_create_listener: result handle={handle}"
        ));
    } else {
        runtime_startup_trace("wr_game_session_create_listener: result=nil");
    }
    result
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_poll_event(listener_handle: Value, timeout_ms: Value) -> Value {
    runtime_startup_trace(format!(
        "wr_game_session_poll_event: listener_handle_int={:?} timeout_ms_int={:?}",
        int_value(listener_handle),
        int_value(timeout_ms)
    ));
    let result = web::game_session_poll_event(listener_handle, timeout_ms);
    runtime_startup_trace(format!(
        "wr_game_session_poll_event: result_type_id={}",
        wr_type_id(result)
    ));
    result
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_accept_connection(listener_handle: Value) -> Value {
    runtime_startup_trace(format!(
        "wr_game_session_accept_connection: listener_handle_int={:?}",
        int_value(listener_handle)
    ));
    let result = web::game_session_accept_connection(listener_handle);
    runtime_startup_trace(format!(
        "wr_game_session_accept_connection: result_type_id={}",
        wr_type_id(result)
    ));
    result
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_read_connection_bytes(
    listener_handle: Value,
    connection_handle: Value,
    max_bytes: Value,
) -> Value {
    web::game_session_read_connection_bytes(listener_handle, connection_handle, max_bytes)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_read_http_request_frame(
    listener_handle: Value,
    connection_handle: Value,
) -> Value {
    web::game_session_read_http_request_frame(listener_handle, connection_handle)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_write_connection_bytes(
    listener_handle: Value,
    connection_handle: Value,
    payload: Value,
) -> Value {
    web::game_session_write_connection_bytes(listener_handle, connection_handle, payload)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_write_http_response_frame(
    listener_handle: Value,
    connection_handle: Value,
    response_frame: Value,
) -> Value {
    web::game_session_write_http_response_frame(listener_handle, connection_handle, response_frame)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_write_http_response_vectored(
    listener_handle: Value,
    connection_handle: Value,
    head_bytes: Value,
    body_bytes: Value,
    ...
    web::game_session_write_http_response_vectored(
        listener_handle,
        connection_handle,
        head_bytes,
        body_bytes,
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_send_file(
    listener_handle: Value,
    connection_handle: Value,
    file_path: Value,
    offset: Value,
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_configure_listener_socket(
    listener_handle: Value,
    reuse_port_enabled: Value,
) -> Value {
    web::game_session_configure_listener_socket(listener_handle, reuse_port_enabled)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_close_connection(
    listener_handle: Value,
    connection_handle: Value,
) -> Value {
    web::game_session_close_connection(listener_handle, connection_handle)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_close_listener(listener_handle: Value) -> Value {
    web::game_session_close_listener(listener_handle)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_read_message(
    listener_handle: Value,
    connection_handle: Value,
) -> Value {
    web::game_session_read_message(listener_handle, connection_handle)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_game_session_write_message(
    listener_handle: Value,
    connection_handle: Value,
    message: Value,
) -> Value {
    web::game_session_write_message(listener_handle, connection_handle, message)
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_spawn(archetype_id: Value) -> Value {
    let mut reg = entity_registry().lock().unwrap();
    let id = reg.spawn(int_value(archetype_id).unwrap_or(0) as u32, 0, 0, 0);
    Value::from_int(id as i64)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_despawn(entity_id: Value) -> Value {
    let mut reg = entity_registry().lock().unwrap();
    if reg.despawn(int_value(entity_id).unwrap_or(0) as u64) {
        Value::from_int(0)
    } else {
        Value::from_int(-1)
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_set_position(entity_id: Value, x: Value, y: Value, z: Value) -> Value {
    let mut reg = entity_registry().lock().unwrap();
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let xv = int_value(x).unwrap_or(0) as i32;
    let yv = int_value(y).unwrap_or(0) as i32;
    let zv = int_value(z).unwrap_or(0) as i32;
    if reg.set_position(eid, xv, yv, zv) {
        Value::from_int(0)
    } else {
        Value::from_int(-1)
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_get_position_x(entity_id: Value) -> Value {
    let reg = entity_registry().lock().unwrap();
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    Value::from_int(reg.get_position(eid).map(|p| p[0] as i64).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_get_position_y(entity_id: Value) -> Value {
    let reg = entity_registry().lock().unwrap();
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    Value::from_int(reg.get_position(eid).map(|p| p[1] as i64).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_get_position_z(entity_id: Value) -> Value {
    let reg = entity_registry().lock().unwrap();
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    Value::from_int(reg.get_position(eid).map(|p| p[2] as i64).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_set_component(entity_id: Value, key: Value, value: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let val = int_value(value).unwrap_or(0);
    let key_str = string::with_string_bytes(key, |b| String::from_utf8_lossy(b).into_owned());
    match key_str {
        Some(k) => {
            let mut reg = entity_registry().lock().unwrap();
            if reg.set_component(eid, &k, val) {
                Value::from_int(0)
            } else {
                Value::from_int(-1)
            }
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_get_component(entity_id: Value, key: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let key_str = string::with_string_bytes(key, |b| String::from_utf8_lossy(b).into_owned());
    match key_str {
        Some(k) => {
            let reg = entity_registry().lock().unwrap();
            Value::from_int(reg.get_component(eid, &k).unwrap_or(-1))
        }
        None => Value::from_int(-1),
    }
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_query_archetype(archetype_id: Value) -> Value {
    let reg = entity_registry().lock().unwrap();
    let aid = int_value(archetype_id).unwrap_or(0) as u32;
    let ids = reg.query_by_archetype(aid);
    let list = list::list_new(0);
    for id in ids {
        list::list_push(list, Value::from_int(id as i64));
    }
    list
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_query_radius(x: Value, y: Value, z: Value, radius: Value) -> Value {
    let reg = entity_registry().lock().unwrap();
    let center = [
        int_value(x).unwrap_or(0) as i32,
        int_value(y).unwrap_or(0) as i32,
        int_value(z).unwrap_or(0) as i32,
    ];
    let r = int_value(radius).unwrap_or(0) as i32;
    let ids = reg.query_in_radius(center, r);
    let list = list::list_new(0);
    for id in ids {
        list::list_push(list, Value::from_int(id as i64));
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_entity_count() -> Value {
    let reg = entity_registry().lock().unwrap();
    Value::from_int(reg.count() as i64)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_audio_play_oneshot(
    sound_id: Value,
    volume: Value,
    pitch: Value,
    pan: Value,
) -> Value {
    let _ = (sound_id, volume, pitch, pan);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_audio_play_loop(sound_id: Value, volume: Value, fade_ms: Value) -> Value {
    let _ = (sound_id, volume, fade_ms);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_audio_stop_loop(handle: Value, fade_ms: Value) -> Value {
    let _ = (handle, fade_ms);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_audio_set_listener(x: Value, y: Value, z: Value) -> Value {
    let _ = (x, y, z);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_audio_set_parameter(name: Value, value: Value) -> Value {
    let _ = (name, value);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_create_body(
    entity_id: Value,
    collider_type: Value,
    mass: Value,
    hx: Value,
    ...
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let ct = int_value(collider_type).unwrap_or(0);
    let m = int_value(mass).unwrap_or(0);
    let hxv = int_value(hx).unwrap_or(0);
    let hyv = int_value(hy).unwrap_or(0);
    let hzv = int_value(hz).unwrap_or(0);
    let collider = if ct == 0 {
        physics::core::ColliderV1::Aabb {
            half_extent_x_milli: hxv,
            half_extent_y_milli: hyv,
            half_extent_z_milli: hzv,
        }
    } else {
        physics::core::ColliderV1::Sphere { radius_milli: hxv }
    };
    let pos = {
        let reg = entity_registry().lock().unwrap();
        reg.get_position(eid).unwrap_or([0, 0, 0])
    };
    let mut world = physics_world().lock().unwrap();
    world.create_body(
        eid,
        collider,
        m,
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_remove_body(entity_id: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let mut world = physics_world().lock().unwrap();
    Value::from_int(if world.remove_body(eid) { 0 } else { -1 })
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_set_velocity(
    entity_id: Value,
    vx: Value,
    vy: Value,
    vz: Value,
    ...
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let vxv = int_value(vx).unwrap_or(0);
    let vyv = int_value(vy).unwrap_or(0);
    let vzv = int_value(vz).unwrap_or(0);
    let mut world = physics_world().lock().unwrap();
    Value::from_int(if world.set_velocity(eid, vxv, vyv, vzv) {
        0
    } else {
        -1
    })
...
#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_get_position_x(entity_id: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let world = physics_world().lock().unwrap();
    Value::from_int(world.get_position(eid).map(|p| p[0]).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_get_position_y(entity_id: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let world = physics_world().lock().unwrap();
    Value::from_int(world.get_position(eid).map(|p| p[1]).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_get_position_z(entity_id: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let world = physics_world().lock().unwrap();
    Value::from_int(world.get_position(eid).map(|p| p[2]).unwrap_or(-1))
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_step(dt_ms: Value) -> Value {
    let dt = int_value(dt_ms).unwrap_or(0).max(0) as u32;
    let mut world = physics_world().lock().unwrap();
    let mut reg = entity_registry().lock().unwrap();
    world.step(dt, &mut reg);
    Value::from_int(0)
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_query_contacts(entity_id: Value) -> Value {
    let eid = int_value(entity_id).unwrap_or(0) as u64;
    let world = physics_world().lock().unwrap();
    let ids = world.query_contacts(eid);
    let result = list::list_new(0);
    for id in ids {
        list::list_push(result, Value::from_int(id as i64));
    }
    result
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_raycast(
    ox: Value,
    oy: Value,
    oz: Value,
    dx: Value,
    ...
    let origin = [
        int_value(ox).unwrap_or(0),
        int_value(oy).unwrap_or(0),
        int_value(oz).unwrap_or(0),
    ];
    let direction = [
        int_value(dx).unwrap_or(0),
        int_value(dy).unwrap_or(0),
        int_value(dz).unwrap_or(0),
    ];
    let md = int_value(max_dist).unwrap_or(0);
    let world = physics_world().lock().unwrap();
    match world.raycast(origin, direction, md) {
        Some((eid, _dist)) => Value::from_int(eid as i64),
        None => Value::from_int(-1),
    }
}

#[unsafe(no_mangle)]
pub extern "C" fn wr_physics_add_breakable_joint(
    parent_id: Value,
    child_id: Value,
    threshold: Value,
) -> Value {
    let p = int_value(parent_id).unwrap_or(0) as u64;
    let c = int_value(child_id).unwrap_or(0) as u64;
    let t = int_value(threshold).unwrap_or(0);
    let mut world = physics_world().lock().unwrap();
    world.add_breakable_joint(p, c, t);
    Value::from_int(0)
}
