...
impl CdcEmitter {
    ...
    pub fn emit_put(&mut self, shard: Vec<u8>, key: Vec<u8>, value: Vec<u8>, version: u64) {
    ...
    pub fn emit_delete(&mut self, shard: Vec<u8>, key: Vec<u8>, version: u64) {
        let commit_seq = self.next_seq();
        self.events.push(CdcEvent {
            commit_seq,
            shard,
    ...
    pub fn page_since(
        ...
        let events: Vec<CdcEvent> = self
            ...
            .filter(|event| {
                shard_filter
                    .map(|shard| event.shard.as_slice() == shard)
                    .unwrap_or(true)
            })
            .take(limit)
            .cloned()
