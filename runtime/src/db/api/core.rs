...
use crate::db::analytics::service::{
    AnalyticsExplain, AnalyticsQueryRequest, AnalyticsQueryResult,
};
...
use crate::db::{
    ...
    close_db, open_db, open_db_with_config, read_point, read_point_consistent,
    read_point_consistent_with_sovereignty, read_range, read_range_consistent,
    read_range_consistent_with_sovereignty, submit_batch, submit_batch_insert_fast, submit_put,
    submit_put_insert_fast, txn_abort, txn_begin, txn_commit, txn_prepare,
};
use std::path::Path;
...
pub fn open_with_config(path: &Path, config: &DbConfig) -> Result<i64, DbError> {
...
pub fn put_authorized_fast(
    ...
    intercept_rpc(identity, RpcClass::ClientInsertFast, || {
        submit_put_insert_fast(handle, namespace, key, value)
    })
    .map_err(|err| DbError::invalid_argument(format!("unauthorized rpc: {}", err.reason)))?
}

pub fn batch(handle: i64, ops: &[BatchOp]) -> Result<u64, DbError> {
    submit_batch(handle, ops)
}

pub fn batch_authorized_fast(
    handle: i64,
    ops: &[BatchOp],
    identity: &CertIdentity,
) -> Result<u64, DbError> {
    intercept_rpc(identity, RpcClass::ClientInsertFast, || {
        submit_batch_insert_fast(handle, ops)
    })
    .map_err(|err| DbError::invalid_argument(format!("unauthorized rpc: {}", err.reason)))?
}
...
pub fn get_consistent_with_sovereignty(
    handle: i64,
    namespace: Vec<u8>,
    key: Vec<u8>,
    consistency: ReadConsistency,
...
pub fn scan_consistent_with_sovereignty(
    handle: i64,
    namespace: Vec<u8>,
    start_key: Vec<u8>,
    end_key: Vec<u8>,
