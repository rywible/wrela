...
/// Typical encoded key fits in 128 bytes (namespace + key with length prefixes).
pub type EncodedUserKey = SmallVec<[u8; 128]>;
...
/// Encodes namespace+key into a stack-friendly buffer to avoid per-op Vec allocation.
/// Use in hot batch prepare paths; falls back to heap when encoded length > 128.
pub fn encode_user_key_smallvec(namespace: &[u8], key: &[u8]) -> Result<EncodedUserKey, DbError> {
    validate_namespace(namespace)?;
    validate_key(key)?;
    let mut out = EncodedUserKey::with_capacity(2 + namespace.len() + 2 + key.len());
    out.extend_from_slice(&(namespace.len() as u16).to_be_bytes());
    out.extend_from_slice(namespace);
    out.extend_from_slice(&(key.len() as u16).to_be_bytes());
    out.extend_from_slice(key);
