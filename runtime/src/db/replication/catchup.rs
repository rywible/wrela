...
pub fn select_transfer_mode(
    follower_match_index: u64,
    leader_last_log_index: u64,
    lag_threshold_ops: u64,
) -> CatchUpTransferMode {
    if lag_threshold_ops == 0 {
        return CatchUpTransferMode::TailLogOnly;
    }
    let lag_ops = leader_last_log_index.saturating_sub(follower_match_index);
    if lag_ops > lag_threshold_ops {
        CatchUpTransferMode::SortedRunThenTail
    } else {
        CatchUpTransferMode::TailLogOnly
    }
}
...
mod tests {
    ...
    #[test]
    fn transfer_mode_prefers_tail_log_when_lag_is_small() {
        assert_eq!(
            select_transfer_mode(95, 100, 16),
            CatchUpTransferMode::TailLogOnly
        );
    }

    #[test]
    fn transfer_mode_switches_to_sorted_run_when_lag_exceeds_threshold() {
        assert_eq!(
            select_transfer_mode(10, 100, 16),
            CatchUpTransferMode::SortedRunThenTail
        );
    }

    #[test]
    fn transfer_mode_disables_sorted_run_when_threshold_is_zero() {
        assert_eq!(
            select_transfer_mode(10, 100, 0),
            CatchUpTransferMode::TailLogOnly
        );
    }
    ...
    #[test]
    fn integration_partial_stream_retry_recovers_and_converges() {
        let mut harness = CatchUpHarness {
            follower_state: NodeState::with_timing(2, 0, 10),
            follower_match_index: 3,
            follower_last_snapshot_index: 3,
            follower_last_snapshot_term: 1,
            leader_term: 6,
            leader_snapshot_index: 20,
            leader_snapshot_term: 7,
            leader_last_log_index: 44,
            leader_current_first_log_index: 1,
            leader_committed_index: 44,
            retention_entries: 10,
            now_tick: 200,
            election_timeout_ticks: 20,
        };
        ...
        let first = harness
            .run_round(None)
            .expect_err("missing payload should fail before any state change");
        assert_eq!(first, CatchUpExecutionError::MissingSnapshotPayload);
        assert_eq!(harness.follower_match_index, 3);

        let second = harness
            .run_round(Some(b"snapshot-after-retry"))
            .expect("retry with payload should recover");
        assert_eq!(
            second.action,
            CatchUpAction::InstallSnapshotThenLog {
                snapshot_index: 20,
        ...
        assert!(second.snapshot_applied);
        assert_eq!(harness.follower_match_index, 44);

        let final_round = harness
            .run_round(Some(b"snapshot-after-retry"))
            .expect("follower should now be up to date");
        assert_eq!(final_round.action, CatchUpAction::UpToDate);
    }
}
