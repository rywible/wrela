...
mod tests {
    ...
    #[test]
    fn two_node_cluster_quorum_equals_two_no_fault_tolerance() {
        let candidates = vec![
            NodeHealth {
                node_id: "a".to_string(),
                region: "us".to_string(),
        ...
        let latency = BTreeMap::from([("a".to_string(), 1_u64), ("b".to_string(), 2_u64)]);

        let selected = select_nearest_healthy_quorum(&candidates, 2, &latency).expect("quorum");
        // quorum = (2/2)+1 = 2 — both nodes needed, zero fault tolerance.
        assert_eq!(selected.quorum_size, 2);
        assert_eq!(selected.selected_nodes.len(), 2);
    }

    #[test]
    fn two_node_cluster_one_unavailable_fails() {
        let candidates = vec![
            NodeHealth {
                node_id: "a".to_string(),
                region: "us".to_string(),
        ...
        let latency = BTreeMap::from([("a".to_string(), 1_u64)]);

        let err = select_nearest_healthy_quorum(&candidates, 2, &latency).expect_err("must fail");
        assert!(
            matches!(
                err,
                QuorumSelectionError::InsufficientHealthyNodes {
                    required: 2,
                    available: 1,
                }
            ),
            "2-node cluster cannot tolerate any failure"
        );
    }

    #[test]
    fn single_node_cluster_degenerates_to_quorum_of_one() {
        let candidates = vec![NodeHealth {
        ...
        let latency = BTreeMap::from([("solo".to_string(), 1_u64)]);

        let selected = select_nearest_healthy_quorum(&candidates, 1, &latency).expect("quorum");
        assert_eq!(selected.quorum_size, 1);
        assert_eq!(selected.selected_nodes, vec!["solo".to_string()]);
    }

    #[test]
    fn all_degraded_quorum_selection_succeeds() {
        let candidates = vec![
            NodeHealth {
                node_id: "a".to_string(),
                region: "us".to_string(),
        ...
        // Degraded is not Unavailable, so quorum should still form.
        let selected = select_nearest_healthy_quorum(&candidates, 3, &latency).expect("quorum");
        assert_eq!(selected.quorum_size, 2);
        // Sorted by latency: b(1), c(2).
        assert_eq!(
            selected.selected_nodes,
    ...
    #[test]
    fn all_degraded_safety_simulation_fails_on_zero_max_degraded() {
        let candidates = vec![
            NodeHealth {
                node_id: "a".to_string(),
                region: "us".to_string(),
        ...
        // max_degraded_selected=0: must fail because all selected are degraded.
        let sim = simulate_quorum_safety(&candidates, 3, &latency, 0, 0).expect("sim");
        assert!(
            !sim.passes,
            "must fail with 0 max_degraded when all degraded"
        );
        assert_eq!(sim.degraded_selected, 2);
    }
