...
pub fn authorize(identity: &CertIdentity, rpc: RpcClass) -> Result<(), AuthzError> {
    ...
    let allowed = match identity.role {
        ...
        MembershipRole::Gateway => matches!(
            rpc,
            RpcClass::ClientRead | RpcClass::ClientWrite | RpcClass::ClientInsertFast
        ),
    };
...
mod tests {
    ...
    fn voter_role_allows_replication_and_client_rw() {
        ...
        assert!(authorize(&id, RpcClass::ClientInsertFast).is_ok());
    ...
    fn learner_role_denies_vote_and_write() {
        ...
        assert!(authorize(&id, RpcClass::ClientInsertFast).is_err());
    }
