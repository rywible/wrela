...
pub enum RecoveryAction {
    ReprepareParticipant { participant_id: u64 },
    RecommitParticipant { participant_id: u64 },
    ReabortParticipant { participant_id: u64 },
    FinalizeCommit,
    FinalizeAbort,
}
    ...
    match record.decision {
        None => {
            for participant_id in &record.participants {
                if !record.prepared.contains(participant_id) {
                    actions.push(RecoveryAction::ReprepareParticipant {
...
mod tests {
    ...
    fn commit_recovery_rebroadcasts_missing_acks() {
        ...
        assert_eq!(
            actions,
            vec![
                RecoveryAction::RecommitParticipant { participant_id: 2 },
                RecoveryAction::RecommitParticipant { participant_id: 3 },
            ]
        );
    }

    #[test]
    fn prepare_recovery_reissues_prepare_for_missing_participants() {
        let record = CoordinatorRecord {
            txn_id: 22,
            epoch: 1,
        ...
        assert_eq!(
            ...
            vec![RecoveryAction::ReprepareParticipant { participant_id: 5 }]
        );
    }
}
