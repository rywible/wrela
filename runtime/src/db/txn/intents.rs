...
impl IntentStore {
    fn acquire_target(
        ...
        // Fast path: check same-txn intents via the by_txn index.
        if let Some(indices) = self.by_txn.get(&txn_id) {
            for &idx in indices {
                if target_equal(&self.intents[idx].target, &target) {
                    return Ok(IntentAcquireResult::AlreadyHeld);
                }
            }
        }

        // Single pass: check cross-txn overlap conflicts.
        ...
        self.by_txn.entry(txn_id).or_default().push(idx);
    ...
    pub fn release_txn(&mut self, txn_id: u64) -> usize {
        ...
        if released > 0 {
            ...
            // Rebuild indices for remaining transactions since Vec positions
            // shifted after retain.
            self.rebuild_by_txn_index();
        }
        released
    }

    fn rebuild_by_txn_index(&mut self) {
        self.by_txn.clear();
        for (idx, intent) in self.intents.iter().enumerate() {
            self.by_txn.entry(intent.txn_id).or_default().push(idx);
        }
    }
...
mod tests {
    ...
    #[test]
    fn single_pass_conflict_detection_matches_original() {
        let mut store = IntentStore::default();

        // Txn 1 acquires key "a" and range "d".."g".
        assert_eq!(
            store.acquire_key(1, b"a".to_vec(), 10),
            Ok(IntentAcquireResult::Acquired)
        );
        assert_eq!(
            store.acquire_range(1, b"d".to_vec(), b"g".to_vec(), 11),
            Ok(IntentAcquireResult::Acquired)
        );

        // Same-txn reentrant check: already-held via by_txn index.
        assert_eq!(
            store.acquire_key(1, b"a".to_vec(), 12),
            Ok(IntentAcquireResult::AlreadyHeld)
        );
        assert_eq!(
            store.acquire_range(1, b"d".to_vec(), b"g".to_vec(), 13),
            Ok(IntentAcquireResult::AlreadyHeld)
        );

        // Cross-txn conflict: txn 2 tries key in txn 1's range.
        let conflict = store
            .acquire_key(2, b"e".to_vec(), 14)
            .expect_err("must conflict with txn 1 range");
        assert_eq!(conflict.holder_txn_id, 1);

        // Cross-txn no-conflict: txn 2 acquires key outside txn 1's range.
        assert_eq!(
            store.acquire_key(2, b"z".to_vec(), 15),
            Ok(IntentAcquireResult::Acquired)
        );

        // Release txn 1, verify txn 2 can now acquire the formerly-held range.
        assert_eq!(store.release_txn(1), 2);
        assert_eq!(
            store.acquire_range(2, b"d".to_vec(), b"g".to_vec(), 16),
            Ok(IntentAcquireResult::Acquired)
        );
    }
}
