...
pub fn handle_install_snapshot(
    ...
    // Raft Section 7: discard all log entries covered by the snapshot and advance commit_index.
    state.truncate_log_from(0);
...
mod tests {
    ...
    fn accepts_newer_term_and_applies_newer_snapshot() {
        ...
        assert!(
            state.log.is_empty(),
            "log must be truncated after snapshot install"
        );
        assert_eq!(
            state.commit_index, 120,
