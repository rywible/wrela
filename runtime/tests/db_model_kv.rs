...
use wrela_runtime::db::api::core as api;
use wrela_runtime::db::types::{BatchOp, ErrorCode};
use wrela_runtime::db::{DbConfig, config::ReplicationConfig};
...
fn test_db() -> (TempDir, i64) {
    ...
    let handle = api::open_with_config(dir.path(), &config).expect("open db");
    (dir, handle)
}
...
fn batch_occ_mismatch_is_all_or_nothing() {
    ...
    let failing_batch = vec![
        BatchOp::Put {
            namespace: namespace.clone().into(),
            key: key_a.clone().into(),
            value: as_bytes("next-a").into(),
            expected_version: Some(v1),
        },
        BatchOp::Put {
            namespace: namespace.clone().into(),
            key: key_b.clone().into(),
            value: as_bytes("bad-b").into(),
