...
def write_viewer(folder,records,art=None):
    ...
    for r in records:
        ...
        items.append(dict(variant=r['variant'],condition=r['condition'],view=r['view'],frame=r['frame'],src=os.path.relpath(r['path'],folder),time=m['time'],shader=m['shaderDigest'],source=m.get('sourceDigest'),imageSHA256=hashlib.sha256(Path(r['path']).read_bytes()).hexdigest(),subject=m['studioObject'],height=m['workshop']['heightMetres'],gpu=m['gpuMilliseconds'].get('median'),document=m['workshop']['document']))
