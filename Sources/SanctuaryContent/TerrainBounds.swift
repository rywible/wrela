...
private struct Interval {
  var lo: Float
  var hi: Float
  init(_ lo: Float, _ hi: Float) {
    self.lo = lo
    self.hi = hi
  }
  static func + (a: Self, b: Self) -> Self { Self(a.lo + b.lo, a.hi + b.hi) }
  static func * (a: Self, b: Float) -> Self {
    b >= 0 ? Self(a.lo * b, a.hi * b) : Self(a.hi * b, a.lo * b)
  }
  static func * (a: Self, b: Self) -> Self {
    let v = [a.lo * b.lo, a.lo * b.hi, a.hi * b.lo, a.hi * b.hi]
    return Self(v.min()!, v.max()!)
  }
  ...
  var sin: Self {
    ...
    var a = min(Foundation.sin(lo), Foundation.sin(hi))
    var b = max(Foundation.sin(lo), Foundation.sin(hi))
    if ceil((lo - Float.pi / 2) / (2 * Float.pi)) <= floor((hi - Float.pi / 2) / (2 * Float.pi)) {
      b = 1
    }
    if ceil((lo + Float.pi / 2) / (2 * Float.pi)) <= floor((hi + Float.pi / 2) / (2 * Float.pi)) {
      a = -1
...
extension Terrain {
  ...
  public func patchCertificate(x: ClosedRange<Float>, z: ClosedRange<Float>) -> (
    ...
    let hx = X + Interval(34, 34)
    let hz = Z + Interval(45, 45)
    let rz = Z + Interval(122, 122)
    ...
    let flank = BernsteinPolynomial([
      x.lowerBound * x.lowerBound / 375, x.lowerBound * x.upperBound / 375,
      x.upperBound * x.upperBound / 375,
    ]).range
    let height =
      phase.sin * 0.8 + (Z * 0.045).sin * 1.25 + phase2.cos * 0.42
      + Interval(max(0, flank.lowerBound), flank.upperBound) + H * 8 + R * wave.squared * 17
    let dx =
      phase.cos.magnitude * 0.044 + phase2.sin.magnitude * 0.0546 + X.magnitude * (2 / 375)
      + (hx * H).magnitude * (16 / 700) + (R * wave * (X * 0.066 + Interval(0.7, 0.7)).cos)
      .magnitude * (17 * 0.9 * 0.066)
    let dz =
      phase.cos.magnitude * 0.0152 + (Z * 0.045).cos.magnitude * 0.05625 + phase2.sin.magnitude
      * 0.0336 + (hz * H).magnitude * (16 / 1400) + (rz * R * wave.squared).magnitude * (34 / 450)
