...
public enum SurfaceCompiler {
    ...
    public static func compile(_ shape:Shape,resolution:Int,color:V3)->(Mesh,SurfaceReport) {
        ...
        var samples:[Int:Float]=[:],cells:[Int:UInt32]=[:],edges=Set<Edge>(),mesh=Mesh(),report=SurfaceReport()
        ...
        func value(_ p:SIMD3<Int>)->Float {
            let k=key(p);if let v=samples[k]{return v}
            let v=shape.value(at:point(p));samples[k]=v;report.fieldEvaluations+=1;return v
        }
        ...
        func leaf(_ p:SIMD3<Int>) {
            let ds=corners.map{value(p &+ $0)}
            ...
            for (a,b) in pairs where (ds[a]<0) != (ds[b]<0) {
                ...
                var lo=point(ga),hi=point(gb),dlo=ds[a],dhi=ds[b]
                // Safeguarded secant iteration retains the sign bracket.
                for _ in 0..<12 {
                    let t=clamp(dlo/(dlo-dhi),0.1,0.9),q=lo+(hi-lo)*t,v=shape.value(at:q)
                    report.fieldEvaluations+=1
                    if (v<0)==(dlo<0){lo=q;dlo=v}else{hi=q;dhi=v}
                }
                let hit=lo+(hi-lo)*clamp(dlo/(dlo-dhi),0,1)
                hits.append(hit);normals.append(shape.normal(at:hit));edges.insert(Edge(key(ga),key(gb)))
            }
            ...
            func consider(_ q:V3) {
                guard all(q .>= lo-V3(repeating:1e-6)),all(q .<= hi+V3(repeating:1e-6)),q.x.isFinite,q.y.isFinite,q.z.isFinite else{return}
                let d=q-mass,error=dot(d,A*d)-2*dot(d,b)
                if error<bestError {best=q;bestError=error}
            }
            // Solve the box-constrained quadratic on interior, faces, edges and corners.
            for sx in -1...1 {for sy in -1...1 {for sz in -1...1 {
                let state=[sx,sy,sz];var M=A,rhs=b,q=V3.zero
                ...
                consider(mass+M.inverse*rhs)
            }}}
            cells[key(p)]=UInt32(mesh.vertices.count)
            mesh.vertices.append(Vertex(best,shape.normal(at:best),color));report.surfaceCells+=1
        }
        func visit(_ p:SIMD3<Int>,_ width:Int) {
            let center=point(p)+step*Float(width)*0.5,radius=length(step*Float(width))*0.5
            report.fieldEvaluations+=1
            if abs(shape.value(at:center))>radius*1.00001+1e-6 {report.excludedCells+=1;return}
            if width==1 {leaf(p);return}
            let half=width/2
            for c in corners {visit(p &+ c &* half,half)}
        }
        visit(.zero,n)
        for e in edges.sorted(by:{$0.a==$1.a ? $0.b<$1.b:$0.a<$1.a}) {
