...
struct AssetSource: Codable {
  ...
  static var availableIDs: [String] {
    ids
      + (((try? FileManager.default.contentsOfDirectory(
        at: url("tree").deletingLastPathComponent(), includingPropertiesForKeys: nil)) ?? []).filter
    { $0.pathExtension == "json" }.map { $0.deletingPathExtension().lastPathComponent }.filter {
      !ids.contains($0)
    }).sorted()
  }
  ...
  static func url(_ id: String) -> URL {
    workspace.appendingPathComponent("Authoring/Assets/\(id).json")
  }
  static let treeRanges: [String: ClosedRange<Float>] = [
    "height": 3...12, "width": 2...10, "trunk": 0.1...0.6, "spread": 0.4...2, "leaves": 200...4000,
  ]
  static func defaults(_ id: String) -> AssetSource {
    AssetSource(
      id: id,
      name: [
        "tree": "Sanctuary tree", "seed": "Seed pod", "stone": "Mossy stone",
        "branch": "Branch structure", "calibration": "Material spheres",
      ][id] ?? id, generator: id,
      parameters: id == "tree" || id == "branch"
        ? ["height": 5.6, "width": 6, "trunk": 0.24, "spread": 1, "leaves": 1800] : [:])
  }
  ...
  func validate() throws {
    ...
    for (key, value) in parameters {
      guard let range = Self.treeRanges[key], value.isFinite, range.contains(value) else {
    ...
    if generator == "fields" {
      guard (1...12).contains(parts.count), parts.reduce(0, { $0 + $1.field.nodeCount }) <= 128
      else {
        throw RuntimeError.message("A field asset needs 1...12 parts and at most 128 field nodes")
      }
      for part in parts {
        guard part.color.count == 3, part.color.allSatisfy({ $0.isFinite && (0...1).contains($0) }),
          (0...8).contains(part.material), (0.05...1).contains(part.roughness),
  ...
  private func compileParts() throws -> [SceneBatch] {
    ...
    default:
      return try parts.enumerated().map { i, p in
        try batch(
          "\(i) · \(p.name)", p.field.shape(), V3(p.color[0], p.color[1], p.color[2]),
...
extension AssetSource {
  enum CodingKeys: String, CodingKey {
    case version, id, name, generator, parameters, parts, appearance
  }
  init(from decoder: Decoder) throws {
    let c = try decoder.container(keyedBy: CodingKeys.self)
