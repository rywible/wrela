...
final class GameView: MTKView {
  ...
  override func keyDown(with event: NSEvent) {
    ...
    if c.soundstageMode && [48, 15, 35, 18, 19, 20, 123, 124, 125, 126].contains(event.keyCode) {
      r.workshopSession.commit()
      r.workshopSession.begin("Keyboard edit")
    }
    defer { r.workshopSession.commit() }
    switch event.keyCode {
    case 14: if !c.soundstageMode && !event.isARepeat { c.interactWithWorld() }
...
final class AppController: NSObject, NSApplicationDelegate, NSWindowDelegate {
  var soundstageMode: Bool {
    Bundle.main.object(forInfoDictionaryKey: "SanctuaryMode") as? String == "soundstage"
  }
  var window: NSWindow!
  var gameView: GameView!
  var renderer: SanctuarySession?
  ...
  var interactionButton: ActionButton?
  var expeditionLabel: NSTextField?
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    let appMenu = NSMenu()
    appMenu.addItem(
      withTitle: "Quit Sanctuary", action: #selector(NSApplication.terminate(_:)),
      keyEquivalent: "q")
    appItem.submenu = appMenu
    NSApp.mainMenu = menu
    window = NSWindow(
      contentRect: NSRect(x: 0, y: 0, width: 1200, height: soundstageMode ? 760 : 675),
    ...
    gameView.setAccessibilityLabel(
      "Field garden. W A S D to walk. Drag to look. Tab switches object studio.")
    if soundstageMode {
    ...
    ).appendingPathComponent(soundstageMode ? ".soundstage" : ".sanctuary")
    do {
      let r = try SanctuarySession(
        view: gameView, world: GardenWorld(soundstageOnly: soundstageMode),
        soundstage: soundstageMode)
      renderer = r
      bridge = try AgentBridge(renderer: r, root: rootURL)
      r.expedition?.onMessage = { [weak self] text in self?.messageLabel.stringValue = text }
      if let expedition = r.expedition { messageLabel.stringValue = expedition.message }
      ...
      let alert = NSAlert()
      alert.messageText = "Sanctuary could not start"
  ...
  func buildHUD() {
    ...
    let header = panel()
    gameView.addSubview(header)
    let eyebrow = label(
      "S A N C T U A R Y", size: 10, weight: .semibold,
      color: NSColor(calibratedRed: 0.83, green: 0.87, blue: 0.68, alpha: 1))
    titleLabel = label("The field garden", size: 25, weight: .medium)
    titleLabel.font =
      NSFont(name: "NewYork-Regular", size: 25) ?? NSFont.systemFont(ofSize: 25, weight: .medium)
    subtitleLabel = label(
      "A small beginning for a living world", size: 11, color: .init(white: 0.85, alpha: 1))
    let headings = NSStackView(views: [eyebrow, titleLabel, subtitleLabel])
    headings.orientation = .vertical
    headings.alignment = .leading
    headings.spacing = 5
    headings.translatesAutoresizingMaskIntoConstraints = false
    header.addSubview(headings)
    NSLayoutConstraint.activate([
      header.leadingAnchor.constraint(equalTo: gameView.leadingAnchor, constant: 24),
      header.topAnchor.constraint(equalTo: gameView.topAnchor, constant: 24),
      headings.leadingAnchor.constraint(equalTo: header.leadingAnchor, constant: 18),
      headings.trailingAnchor.constraint(equalTo: header.trailingAnchor, constant: -22),
      headings.topAnchor.constraint(equalTo: header.topAnchor, constant: 14),
      headings.bottomAnchor.constraint(equalTo: header.bottomAnchor, constant: -16),
    ])

    notes = panel()
    gameView.addSubview(notes)
    let noteTitle = label(
      soundstageMode ? "SOUNDSTAGE" : "FIELD NOTES", size: 10, weight: .semibold,
      color: .init(white: 0.7, alpha: 1))
    sceneButton = ActionButton("Open object studio") { [weak self] in self?.toggleScene() }
    let presets = NSStackView(views: [
      button("Noon") { [weak self] in self?.renderer?.lighting = "morning" },
      button("Sunset") { [weak self] in self?.renderer?.lighting = "sunset" },
      button("Cloud") { [weak self] in self?.renderer?.lighting = "overcast" },
    ])
    presets.spacing = 4
    let weather = NSStackView(views: [
      button("Rain") { [weak self] in self?.renderer?.lighting = "rain" },
      button("Indoors") { [weak self] in self?.renderer?.lighting = "indoor" },
      button("Wet / dry") { [weak self] in
        guard let r = self?.renderer else { return }
        r.wetness = r.wetness > 0.5 ? 0 : 1
      },
    ])
    weather.spacing = 4
    let subjects = NSStackView(views: [
      button("Seed") { [weak self] in try? self?.renderer?.selectSubject("seed") },
      button("Stone") { [weak self] in try? self?.renderer?.selectSubject("stone") },
      button("Branch") { [weak self] in try? self?.renderer?.selectSubject("branch") },
    ])
    subjects.spacing = 4
    scaleLabel = label("Seed pod · scale 1.00", size: 12)
    scaleSlider = NSSlider(
      value: 1, minValue: 0.6, maxValue: 1.7, target: self, action: #selector(scaleChanged))
    scaleSlider.isContinuous = true
    scaleSlider.setAccessibilityLabel("Seed pod scale")
    let cameraButtons = NSStackView(views: [
      button(soundstageMode ? "Front" : "Arrival") { [weak self] in self?.setCamera("arrival") },
      button(soundstageMode ? "Quarter" : "Meadow") { [weak self] in self?.setCamera("meadow") },
      button(soundstageMode ? "Above" : "Gate") { [weak self] in self?.setCamera("gate") },
    ])
    cameraButtons.spacing = 4
    ...
    statsLabel = label("Compiling the garden…", size: 10, color: .init(white: 0.76, alpha: 1))
    statsLabel.maximumNumberOfLines = 3
    let skyViews = NSStackView(views: [
      button("Sunward") { [weak self] in self?.renderer?.setStudyView("sunward") },
      button("Away") { [weak self] in self?.renderer?.setStudyView("away") },
      button("Zenith") { [weak self] in self?.renderer?.setStudyView("zenith") },
    ])
    skyViews.spacing = 4
    let twilight = NSStackView(views: [
      button("Golden") { [weak self] in self?.renderer?.lighting = "golden" },
      button("Afterglow") { [weak self] in self?.renderer?.lighting = "afterglow" },
    ])
    twilight.spacing = 4
    let stack = NSStackView(views: [
      noteTitle, sceneButton, label("LIGHT", size: 9, weight: .medium, color: .gray), presets,
      twilight, weather, subjects, scaleLabel, scaleSlider,
      label(soundstageMode ? "VIEWS" : "WAYPOINTS", size: 9, weight: .medium, color: .gray),
      cameraButtons, skyViews,
      button("Live / pause") { [weak self] in self?.renderer?.paused.toggle() }, capture, reset,
      statsLabel,
    ])
    ...
    NSLayoutConstraint.activate([
      notes.trailingAnchor.constraint(equalTo: gameView.trailingAnchor, constant: -24),
      notes.topAnchor.constraint(equalTo: gameView.topAnchor, constant: 24),
      notes.widthAnchor.constraint(equalToConstant: 244),
      stack.leadingAnchor.constraint(equalTo: notes.leadingAnchor, constant: 16),
      stack.trailingAnchor.constraint(equalTo: notes.trailingAnchor, constant: -16),
      stack.topAnchor.constraint(equalTo: notes.topAnchor, constant: 16),
      stack.bottomAnchor.constraint(equalTo: notes.bottomAnchor, constant: -16),
      scaleSlider.widthAnchor.constraint(equalToConstant: 210),
    ])

    let footer = panel()
    gameView.addSubview(footer)
    let controls = label(
      soundstageMode
        ? "Drag  orbit     Scroll  zoom     Tab  sky / object     P  live / pause     F  capture     H  notes"
        : "W A S D  walk     ⇧  run     Drag / arrows  look     E  interact     H  tools", size: 11,
      weight: .medium)
    messageLabel = label(
      soundstageMode
        ? "A separate space for field, material, and sky studies."
        : "Follow the path toward the old gate.", size: 10, color: .init(white: 0.76, alpha: 1))
    interactionButton = button("E · Explore the northern path") { [weak self] in
      self?.interactWithWorld()
    }
    expeditionLabel = label(
      "", size: 12, weight: .medium,
      color: NSColor(calibratedRed: 0.72, green: 0.88, blue: 0.90, alpha: 1))
    let footerStack = NSStackView(views: [
      expeditionLabel!, interactionButton!, controls, messageLabel,
    ])
    ...
    NSLayoutConstraint.activate([
      footer.leadingAnchor.constraint(equalTo: gameView.leadingAnchor, constant: 24),
      footer.bottomAnchor.constraint(equalTo: gameView.bottomAnchor, constant: -20),
    ...
    notes.isHidden = true
    crosshair = label("·", size: 24, weight: .light, color: .init(white: 1, alpha: 0.65))
    gameView.addSubview(crosshair)
    NSLayoutConstraint.activate([
      crosshair.centerXAnchor.constraint(equalTo: gameView.centerXAnchor),
      crosshair.centerYAnchor.constraint(equalTo: gameView.centerYAnchor),
    ])
  }
  ...
  @objc func scaleChanged() {
    renderer?.podScale = scaleSlider.floatValue
    updateHUD()
    window.makeFirstResponder(gameView)
  }
  func toggleScene() {
    ...
    r.scene = r.scene == "garden" ? "studio" : "garden"
    r.keys.removeAll()
    updateHUD()
    window.makeFirstResponder(gameView)
  }
  ...
  func setCamera(_ name: String) {
    guard let r = renderer else { return }
    if soundstageMode {
      r.setStudyView(["arrival": "front", "meadow": "quarter", "gate": "above"][name] ?? "front")
      updateHUD()
      return
    }
    r.scene = "garden"
    ...
    case "meadow":
      ...
      r.pitch = -0.03
    case "gate":
      r.camera = V3(r.world.terrain.pathX(-48), 0, -48)
      r.yaw = 0
      r.pitch = 0.12
  ...
  func updateHUD() {
    ...
    titleLabel.stringValue =
      studio
      ? (r.studioObject == "sky" ? "Atmosphere studies" : "Material studies") : "The first resident"
    subtitleLabel.stringValue =
      studio ? "One field · many ways of seeing" : "A small beginning for a living world"
    if let expedition = r.expedition {
      r.syncExpeditionPlayer()
      let state = expedition.state
      subtitleLabel.stringValue = studio ? "One field · many ways of seeing" : state.objective
      ...
      interactionButton?.title =
        ...
        + (state.prompt
          ?? (state.phase == .carrying
            ...
            : state.phase == .settled ? "Visit your frost garden" : "Explore the northern path"))
      interactionButton?.isEnabled = !studio && state.prompt != nil
      interactionButton?.setAccessibilityLabel(interactionButton!.title)
      interactionButton?.isHidden = studio
      expeditionLabel?.isHidden = studio
      if state.phase == .carrying {
        let d = Expedition.home - state.player
        let direction =
          abs(d.y) > abs(d.x) ? (d.y > 0 ? "south" : "north") : (d.x > 0 ? "east" : "west")
        expeditionLabel?.stringValue = String(
          format: "Frostling safe in your care · Home %.0f m %@", length(d), direction)
      } else if state.phase == .settled {
        expeditionLabel?.stringValue = String(
          format: "Frost garden · %.0f%% established · 1 resident", state.habitat * 100)
      } else {
        expeditionLabel?.stringValue =
    ...
    sceneButton.title =
      soundstageMode
      ? (r.studioObject == "sky" ? "Return to object" : "Study the sky")
      : (studio ? "Return to the garden" : "Open object studio")
    sceneButton.setAccessibilityLabel(sceneButton.title)
    scaleLabel.stringValue = String(
      format: "Seed pod · %.0f mm tall", GardenWorld.podMetres * r.podScale * 1000)
    scaleSlider.floatValue = r.podScale
    let gpu = r.stats(r.gpuTimes)["median"] ?? 0
    statsLabel.stringValue = String(
      format: "1920 × 1080 · native Metal\nGPU %.1f ms · %@ · %@", gpu, r.lighting,
      r.paused ? "paused" : "live")
  }
  func interactWithWorld() {
    do { messageLabel.stringValue = try renderer?.interact() ?? "" } catch {
  ...
  func applicationWillTerminate(_ notification: Notification) {
    ...
    renderer?.syncExpeditionPlayer()
    if let expedition = renderer?.expedition {
      do { try expedition.save() } catch { fputs("Expedition save failed: \(error)\n", stderr) }
    }
...
enum SanctuaryMain {
  static func main() {
    if CommandLine.arguments.contains("--check-renderer") {
      do {
        let r = try SanctuarySession(
          view: MTKView(), world: GardenWorld(soundstageOnly: true), soundstage: true)
        print("Renderer compiled on \(r.device.name)")
      } catch {
        fputs("\(error)\n", stderr)
        exit(1)
      }
