...
extension SanctuarySession {
  ...
  func applySource(_ source: AssetSource) throws {
    ...
    for batch in prepared {
      ...
      for i in 0..<batch.vertices.length / MemoryLayout<Vertex>.stride {
        let point = batch.sourceInstances[0].model * vertices[i].position
  ...
  func editShape(_ values: [String: Float]) throws {
    guard studioSource.generator == "tree" || studioSource.generator == "branch" else {
  ...
  func workshopDocument(includeWind: Bool = true) -> WorkshopDocument {
    WorkshopDocument(
      source: studioSource, skyOnly: studioObject == "sky", rig: studioRig, layout: studioLayout,
      lighting: lighting,
      sky: [
  ...
  func restoreWorkshop(_ d: WorkshopDocument) throws {
    ...
    importedSourceURL = d.sourcePath.map { URL(fileURLWithPath: $0) }
  ...
  func workshopSnapshot() -> [String: Any] {
    ...
    let document =
      ...
      ?? [:]
    return [
      "authoring": workshopSession.snapshot, "document": document,
  ...
  static var workshopCatalog: [String: Any] {
    [
      ...
      "views": [
        ...
        "away", "zenith",
      ], "shapeRanges": AssetSource.treeRanges.mapValues { [$0.lowerBound, $0.upperBound] },
      "rigRanges": [
      ...
      "artDirection": "Authoring/ArtDirection.json",
      "fieldOperations": [
