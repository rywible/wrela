...
final class WorkshopPanel: NSView {
  ...
  init(_ owner: AppController) {
    ...
    for name in ["Object", "Lighting", "Look", "Files"] {
      navigation.addArrangedSubview(
        ActionButton(name) { [weak self] in
    ...
    func button(_ name: String, _ action: @escaping (SanctuarySession) throws -> Void)
      -> ActionButton
    {
    ...
    func slider(
      ...
      _ action: @escaping (SanctuarySession, Float) throws -> Void
    ) -> NSStackView {
      ...
      let number = WorkshopNumber(title) { [weak self] value in
        ...
        guard let parsed, parsed.isFinite, range.contains(parsed) || inherited else {
          owner.messageLabel.stringValue =
            "\(title) must be \(range.lowerBound)…\(range.upperBound)."
          self.refresh()
          return
        }
      ...
      let slider = WorkshopSlider(title, value, range) { [weak self] value in
        self?.perform(title) { try action($0, value) }
      }
      let reset = ActionButton("↺") { [weak self] in
        self?.perform("Reset " + title) { r in
    ...
    for (key, title) in [
      ("height", "Growth height · m"), ("width", "Crown width · m"), ("trunk", "Trunk radius · m"),
      ("spread", "Branch spread"), ("leaves", "Leaf count"),
    ] {
      shapeGroup.addArrangedSubview(
        slider(
          title, "shape." + key, AssetSource.defaults("tree").parameters[key]!,
          AssetSource.treeRanges[key]!
        ) { r, v in try r.editShape([key: key == "leaves" ? v.rounded() : v]) })
    }
    add(shapeGroup)
  ...
  required init?(coder: NSCoder) { fatalError() }
  func perform(_ name: String = "Edit", _ action: (SanctuarySession) throws -> Void) {
  ...
  func toolbar() -> NSStackView {
    func b(_ title: String, _ action: @escaping (SanctuarySession) throws -> Void) -> ActionButton {
  ...
  func study(save: Bool) {
    ...
    if save {
      ...
      panel.nameFieldStringValue = "tree-study.json"
  ...
  func refresh() {
    ...
    for id in AssetSource.availableIDs where !subject.itemTitles.contains(id) {
      subject.addItem(withTitle: id)
    }
    ...
    shapeGroup.isHidden =
      activeTab != "Object"
      || !(r.studioSource.generator == "tree" || r.studioSource.generator == "branch")
      || r.studioObject == "sky"
...
extension AppController {
  func buildWorkshopHUD() {
    ...
    let title = label("S A N C T U A R Y  /  WORKSHOP", size: 12, weight: .semibold)
