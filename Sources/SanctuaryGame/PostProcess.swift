...
final class PostProcess {
    ...
    init(device:MTLDevice,library:MTLLibrary) throws {
        func texture(_ w:Int,_ h:Int,_ samples:Int=1)->MTLTexture {
            let d=MTLTextureDescriptor.texture2DDescriptor(pixelFormat:.rgba16Float,width:w,height:h,mipmapped:false)
            d.storageMode = .private;d.usage=[.renderTarget,.shaderRead,.shaderWrite]
            if samples>1 {d.storageMode = device.hasUnifiedMemory ? .memoryless:.private;d.textureType = .type2DMultisample;d.sampleCount=samples;d.usage = .renderTarget}
            return device.makeTexture(descriptor:d)!
        }
        lightingPipeline=try device.makeComputePipelineState(function:library.makeFunction(name:"sceneLightState")!)
        lightStates=(0..<3).map{_ in device.makeBuffer(length:32,options:.storageModePrivate)!}
        hdr=texture(1920,1080);multisample=texture(1920,1080,4)
        bloomA=texture(480,270);bloomB=texture(480,270)
        extract=try device.makeComputePipelineState(function:library.makeFunction(name:"bloomExtract")!)
        blur=try device.makeComputePipelineState(function:library.makeFunction(name:"bloomBlur")!)
        let d=MTLRenderPipelineDescriptor();d.vertexFunction=library.makeFunction(name:"skyVertex");d.fragmentFunction=library.makeFunction(name:"displayComposite");d.colorAttachments[0].pixelFormat = .bgra8Unorm_srgb
        composite=try device.makeRenderPipelineState(descriptor:d)
    }
    func encodeLighting(_ cb:MTLCommandBuffer,uniforms:inout Uniforms,atmosphere:Atmosphere,index:Int,profile:GPUProfile?)->MTLBuffer {
        let state=lightStates[index],e=profile?.compute(cb,"lighting.state") ?? cb.makeComputeCommandEncoder()!
        e.setComputePipelineState(lightingPipeline)
        for (i,t) in [atmosphere.trans,atmosphere.previousSky,atmosphere.sky,atmosphere.cloudAmbient].enumerated(){e.setTexture(t,index:i)}
        e.setBytes(&uniforms,length:MemoryLayout<Uniforms>.stride,index:0);e.setBuffer(state,offset:0,index:1)
    ...
    func encode(_ cb:MTLCommandBuffer,to output:MTLTexture,lightState:MTLBuffer,look:SIMD4<Float>,profile:GPUProfile?=nil) {
        func compute(_ pipeline:MTLComputePipelineState,_ source:MTLTexture,_ target:MTLTexture,_ direction:SIMD2<Int32> = .zero) {
            ...
            e.dispatchThreads(MTLSize(width:480,height:270,depth:1),threadsPerThreadgroup:MTLSize(width:8,height:8,depth:1));e.endEncoding()
        }
        compute(extract,hdr,bloomA);compute(blur,bloomA,bloomB,SIMD2(1,0));compute(blur,bloomB,bloomA,SIMD2(0,1))
