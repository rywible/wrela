...
final class AgentBridge {
  ...
  init(renderer: SanctuarySession, root: URL) throws {
  ...
  func execute(_ c: [String: Any], id: String) throws {
    ...
    case "interact": onMessage?(try renderer.interact())
    case "saveExpedition":
      guard let expedition = renderer.expedition else {
        throw RuntimeError.message("No game expedition in the soundstage")
      }
      renderer.syncExpeditionPlayer()
      try expedition.save()
    case "expeditionSlot":
      guard let name = c["name"] as? String else {
        throw RuntimeError.message("A save slot name is required")
      }
      try renderer.loadExpeditionSlot(name)
    case "status": break
    case "scene":
      ...
      guard !renderer.isSoundstage || s == "studio" else {
        throw RuntimeError.message(
          "The soundstage contains no garden; use Sanctuary for playtesting")
      }
      renderer.scene = s
    ...
    case "catalog":
      respond(id, ["ok": true, "catalog": SanctuarySession.workshopCatalog])
      return
    case "rig":
    ...
    case "shape":
      ...
      for key in c.keys where key != "action" && key != "id" {
        guard AssetSource.treeRanges[key] != nil else {
    ...
    case "reloadAssets":
      ...
      renderer.world = candidate
      renderer.batches = uploaded
      if renderer.scene == "studio",
        FileManager.default.fileExists(atPath: renderer.studioSourceURL.path)
      {
        try renderer.applySource(AssetSource.read(renderer.studioSourceURL))
      }
    case "profiling":
      guard let value = c["value"] as? Bool else {
    ...
    case "parameters":
      ...
      let scale = try number(c, "scale", renderer.podScale)
      ...
      renderer.wind = wind
      renderer.podScale = scale
      if c["scale"] != nil && renderer.scene == "studio" && renderer.studioObject == "seed" {
    ...
    case "query":
      ...
      guard abs(p.x) <= 10000, abs(p.y) <= 10000, abs(p.z) <= 10000 else {
        throw RuntimeError.message("Query coordinates must be within 10,000 metres of the origin")
      }
      let d =
        renderer.world.studioShape.value(
          at: (p - renderer.podPosition) / (renderer.podScale * GardenWorld.podUnitScale))
        * (renderer.podScale * GardenWorld.podUnitScale)
      respond(
        id,
        [
          "ok": true, "terrainValue": renderer.world.terrain.value(at: p),
          "terrainHeight": renderer.world.terrain.height(p.x, p.z), "terrainSemantics": "implicit",
          "podDistanceBound": d,
          "podPosition": [renderer.podPosition.x, renderer.podPosition.y, renderer.podPosition.z],
        ])
      return
    ...
    case "reloadShaders":
      try renderer.buildPipelines(
        sourceURL: root.deletingLastPathComponent().appendingPathComponent(
          "Sources/SanctuaryGame/Resources/Garden.metal"))
      renderer.clearMetrics()
