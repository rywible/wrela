...
final class MetalRenderer {
  ...
  var grassMeshPipeline: MTLRenderPipelineState!
  var grassVertexPipeline: MTLRenderPipelineState!
  ...
  var metadataProvider: (() -> [String: Any])?
  private var scene = ""
  ...
  func uploadGrass(_ batch: SceneBatch) -> GPUBatch {
  ...
  func buildPipelines(sourceURL: URL? = nil) throws {
    var source = try String(
      contentsOf: sourceURL ?? Bundle.main.url(forResource: "Garden", withExtension: "metal")
        ?? Bundle.module.url(forResource: "Garden", withExtension: "metal")!, encoding: .utf8)
    let atmosphereURL =
    ...
    md.objectFunction = library.makeFunction(name: "gardenObject")
    md.maxTotalThreadsPerObjectThreadgroup = 32
    md.payloadMemoryLength = 132
    md.meshFunction = library.makeFunction(name: "gardenMesh")
  ...
  func draw(_ input: RenderFrame, elapsed: Double, simulationMilliseconds: Double) {
    scene = input.id
    camera = input.camera
    wind = input.wind
  ...
  func renderObjects(_ items: [RenderItem], _ enc: MTLRenderCommandEncoder, shadow: Bool) {
